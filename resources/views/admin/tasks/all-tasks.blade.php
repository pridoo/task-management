    ...
    <div class="min-h-screen bg-gray-100" x-data="{ 
        ...
        <main class="pt-24 px-6 ml-64">
                ...
                @foreach ($tasks as $task)
                    <div class="bg-white shadow rounded-lg p-4 border relative">
                        ...
                        <div class="flex justify-between items-center mb-2">
                            <span
                                class="bg-{{ $task->status == 'To do' ? 'red' : ($task->status == 'In-progress' ? 'yellow' : 'green') }}-500 text-white px-2 py-0.5 rounded-full text-[11px]">{{ $task->status }}</span>
                            <span
                                class="text-xs font-semibold text-{{ $task->priority === 'High' ? 'red' : ($task->priority === 'Medium' ? 'yellow' : 'green') }}-500">
                            ...
                            <div class="flex items-center mt-1 space-x-2">
                                @foreach($task->users as $user)
                                    ...
                                    <div class="relative group" title="{{ $user->name }}">
                                        <div
                                            class="w-7 h-7 flex items-center justify-center text-white text-xs font-semibold rounded-full bg-{{ $bg }}-500">
                                            {{ $initial }}
                                        </div>
            ...
            <div id="editTaskModal"
                ...
                <div
                    class="w-[90%] sm:w-[480px] max-h-[90vh] overflow-hidden bg-white rounded-xl shadow-xl border border-gray-200">
                    <div class="p-6 overflow-y-auto max-h-[80vh] modal-scroll">
                        ...
                        <!-- Edit Task Form -->
                        <form method="POST" id="editTaskForm" enctype="multipart/form-data" class="space-y-4 text-sm"
                            action="">
                            @csrf
                            @method('PUT')
